...
app.get("/health", (req, res) => {
  let url = `http://newsapi.org/v2/top-headlines?category=health&country=us&apiKey=${NEWS_API}`;
  let healthArray = [];
...
function Health(data) {
  ...
  if (data.urlToImage) {
    ...
    this.urlToImage = `https://www.phoneworld.com.pk/wp-content/uploads/2020/03/Digitization-health-sector-Pakistan.jpg`;
  }
...
app.get("/tech", (req, res) => {
  let url = `http://newsapi.org/v2/top-headlines?category=technology&country=us&apiKey=${NEWS_API}`;
  let techArray = [];
  superagent(url).then((result) => {
...
function Tech(data) {
  ...
  if (data.urlToImage) {
    ...
    this.urlToImage = `https://i.nextmedia.com.au/News/CRN_690_coding.jpg`;
  }
...
app.post("/addfavorite", (req, res) => {
  let { category, urlToImage, author, title, url, publishedAt, content } =
    req.body;
...
app.get("/sport", (req, res) => {
  let url = `http://newsapi.org/v2/top-headlines?category=sport&country=us&apiKey=${NEWS_API}`;
  let sportArray = [];
  superagent(url).then((result) => {
...
function Sport(data) {
  ...
  if (data.urlToImage) {
    ...
    this.urlToImage = `https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcR68KlBNlBfL6n8ujQTRzl6X0YBTIcmLhDXEQ&usqp=CAU`;
  }
  this.author = data.author;
  if (data.content) {
    this.content = data.content.slice(0, data.content.indexOf("["));
...
app.get("/index", (request, response) => {
  ...
  let url = `https://api.weatherbit.io/v2.0/forecast/daily?city=amman&key=${WEATHERBIT_KEY}`;
  let newWeather = [];
...
app.post("/signingup", (req, res) => {
  ...
  let val2 = [user_name];
  client.query(SQL2, val2).then((userResults) => {
    if (userResults.rows.length != 0) {
      res.render("welcome", { name: "This username is used", user: "" });
    } else {
      let SQL =
        "INSERT INTO users (user_name,password,userimg,email,first_name,last_name) VALUES ($1,$2,$3,$4,$5,$6);";
